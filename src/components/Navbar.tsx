          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              <div className="relative" ref={dropdownRef}>
                <button onClick={() => setDropdownOpen(!dropdownOpen)} className="w-9 h-9 rounded-full bg-[#1B4F8A] dark:bg-blue-600 flex items-center justify-center text-white text-sm font-bold" title={user.email}>{avatarLetter}</button>
                {dropdownOpen && (
                  <div className="absolute right-0 top-11 w-48 bg-white dark:bg-slate-900 border border-[#DDE5EE] dark:border-slate-700 rounded-xl shadow-lg py-1 z-50">
                    ...
                    {dropdownItems.map((item) => (
                      <Link key={item.href} href={item.href} onClick={() => setDropdownOpen(false)} className="flex items-center gap-2 px-4 py-2 text-sm text-[#3D5166] dark:text-slate-300 hover:text-[#1B4F8A] dark:hover:text-blue-400 hover:bg-[#EBF2FA] dark:hover:bg-slate-700"><span>{item.icon}</span><span>{item.label}</span></Link>
                    ))}
              ...
              <div className="flex items-center gap-2">
                <Link href={ROUTES.AUTH} className="text-xs sm:text-sm py-1.5 px-2.5 sm:px-4 border border-[#C5D3E0] dark:border-slate-600 text-[#4B6080] dark:text-slate-300 rounded-lg hover:border-[#1B4F8A] dark:hover:border-blue-400">登录</Link>
                <Link href={ROUTES.AUTH_REGISTER} className="text-xs sm:text-sm py-1.5 px-2.5 sm:px-4 rounded-lg text-white font-medium bg-[#1B4F8A] dark:bg-blue-600">免费注册</Link>
              </div>
            )}
          </div>
        </div>
