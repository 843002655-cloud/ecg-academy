...
interface Props { texts: string[]; speed?: number; pause?: number; className?: string; }

export default function Typewriter({ texts, speed = 40, pause = 2000, className = "" }: Props) {
  ...
  const currentText = texts[textIndex] ? texts[textIndex].slice(0, charIndex) : "";
  const allComplete = textIndex >= texts.length;

  return (
    <div className={className}>
      {displayed.map((t, i) => (<div key={i} className="mb-1 opacity-80">{t}</div>))}
      {!allComplete && (
        <div className="mb-1">
          {currentText}
