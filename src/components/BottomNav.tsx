  ...
  return (
    <nav aria-label="底部导航" className="md:hidden bottom-nav flex items-center justify-around px-2 bg-white dark:bg-slate-900 border-t border-[#E8ECF0] dark:border-slate-700">
      {tabs.map(({ href, label, Icon }) => {
        ...
        return (
          <Link key={href} href={href} className={`flex flex-col items-center gap-0.5 py-1.5 px-3 min-w-[48px] transition-colors duration-200 ${active ? "text-[#1B4F8A] dark:text-blue-400" : "text-[#8FA0B4] dark:text-slate-500"}`}>
            <Icon size={22} />
