...
import { catColors, diffColors } from "@/lib/constants";
import {
...
export default function CaseDetailPage() {
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    if (typeof window !== "undefined") {
      try { const saved = sessionStorage.getItem(`ecg_chat_${caseId}`); return saved ? JSON.parse(saved) : []; } catch { return []; }
    }
    return [];
  });
  ...
  const [feedbackMap, setFeedbackMap] = useState<Record<number, "up" | "down">>(() => {
    if (typeof window !== "undefined") {
      try { const saved = localStorage.getItem(`ecg_feedback_${caseId}`); return saved ? JSON.parse(saved) : {}; } catch { return {}; }
    }
    return {};
  });
    ...
    caseService.getCaseById(caseId).then((c) => {
      ...
      if (content && typeof content === "object") {
        ...
        if (figsFromPdf && figsFromPdf.length > 0) {
          ...
          figsFromPdf.forEach((fig, i) => { if (!fig.image_url && imgUrls[i]) fig.image_url = imgUrls[i]; });
          extracted.push(...figsFromPdf);
        }
        const imgUrls = (content.image_urls as string[]) || [];
        if (extracted.length === 0 && imgUrls.length > 0) {
          imgUrls.forEach((url, i) => extracted.push({
            figure_number: `图${i + 1}`, title: `心电图 ${i + 1}`,
            description: "", teaching_points: "请观察图中的心电图特征",
            key_question: "你在这份心电图中观察到了什么？请描述关键特征。",
            image_url: url,
          }));
        }
      }
      const introFigure: Figure = {
        figure_number: "病例背景",
        title: "患者信息与病史",
        description: patientInfo || (c as Case).description,
  ...
  const jumpToFigure = async (idx: number) => {
    ...
    try {
      ...
      setMessages((prev) => {
        const next = [...prev];
        next[placeholderIdx] = { role: "assistant", content: `${header}\n\n${body}` };
        return next;
      });
      setStreamingText(null);
    } catch {
  ...
  const handleSend = () => { if (!input.trim() || sending) return; sendMessageToAI(input); setInput(""); };
  const handleHint = () => { if (sending) return; sendMessageToAI("请给我一些提示"); };
  const handleEvaluate = () => { if (sending || messages.length < 3) return; sendMessageToAI("请对我的表现做一个结构化评估，告诉我哪些方面做得好，哪些方面需要加强。"); };

  const handleRestart = () => {
    if (figures.length === 0) return;
    const f = figures[0]; setFigIdx(0);
    ...
    setMessages([{ role: "assistant", content: `${sourceLine2}欢迎来到心电学堂判读室。\n\n今天我们一起分析：**${caseData!.title}**\n\n${figures.length > 0 ? `我们将按步骤分析，共 ${figures.length} 个关键发现。` : ""}\n\n📷 **${f.figure_number}: ${f.title}**\n\n${f.description ? "📖 " + f.description + "\n\n" : ""}🎯 ${f.teaching_points}\n\n${f.key_question}` }]);
    setAllDone(false); setStreamingText(null); setFeedbackMap({});
    if (typeof window !== "undefined") {
      try { localStorage.removeItem(`ecg_feedback_${caseId}`); } catch {}
      try { sessionStorage.removeItem(`ecg_chat_${caseId}`); } catch {}
    }
  };
  ...
  const handleFeedback = (msgIndex: number, type: "up" | "down") => {
    setFeedbackMap((prev) => {
      ...
      if (typeof window !== "undefined") {
        try { localStorage.setItem(`ecg_feedback_${caseId}`, JSON.stringify(next)); } catch {}
      }
      return next;
    });
  };
  ...
  const handleCopyConversation = () => {
    const text = messages.filter((m) => !m._uiOnly).map((m) => `${m.role === "user" ? "👤 学员" : "📈 AI导师"}:\n${m.content}`).join("\n\n---\n\n");
    navigator.clipboard.writeText(text).then(() => { setCopied(true); setTimeout(() => setCopied(false), 1500); }, () => {});
  };

  const handleNextFigure = () => { const next = figIdx + 1; if (next >= figures.length) { setAllDone(true); return; } jumpToFigure(next); };

  const handleChatScroll = () => { const el = chatRef.current; if (!el) return; isAtBottomRef.current = el.scrollHeight - el.scrollTop - el.clientHeight < 50; };
  useEffect(() => { if (isAtBottomRef.current && chatRef.current) chatRef.current.scrollTo(0, chatRef.current.scrollHeight); }, [messages, streamingText]);
  useEffect(() => {
  ...
  useEffect(() => { if (messages.length > 0 && typeof window !== "undefined") try { sessionStorage.setItem(`ecg_chat_${caseId}`, JSON.stringify(messages)); } catch {} }, [messages, caseId]);
  useEffect(() => { if (!lightboxImg) return; const onKey = (e: KeyboardEvent) => { if (e.key === "Escape") setLightboxImg(null); }; window.addEventListener("keydown", onKey); return () => window.removeEventListener("keydown", onKey); }, [lightboxImg]);
  ...
  return (
    <AppLayout>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="card mb-4">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${catColors[caseData.category] || catColors.SVT}`}>{caseData.category}</span>
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${diffColors[caseData.difficulty] || ""}`}>{caseData.difficulty}</span>
          </div>
        ...
        {allDone && (
          <div className="card mb-6">
            <div className="text-center mb-4"><div className="text-4xl mb-2">🎉</div><h2 className="text-xl font-bold text-[#1A2332] dark:text-slate-100 font-serif">学习完成！</h2><p className="text-sm text-[#6B7F96] dark:text-slate-400">你已经完成了本病例全部心电图的分析</p></div>
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              {((caseData.content_json?.key_points as string[]) || caseData.key_points || []).map((kp: string, i: number) => (
                <div key={i} className="flex items-start gap-2 text-sm text-[#3D5166] dark:text-slate-300"><span className="text-[#2D8C6A] dark:text-emerald-400 font-bold mt-0.5">{i + 1}.</span><span>{kp}</span></div>
              ))}
            </div>
            <button onClick={() => { setAllDone(false); jumpToFigure(0); }} className="mt-4 w-full sm:w-auto px-6 py-2.5 bg-[#2D8C6A] dark:bg-emerald-600 text-white font-medium rounded-lg hover:bg-[#1A6B4F] dark:hover:bg-emerald-500 transition-colors">重新学习</button>
          </div>
        )}
                  ...
                  {figures.map((f, i) => (
                    <button key={i} onClick={() => void jumpToFigure(i)} disabled={introGenerating}
                      className={`shrink-0 px-2.5 py-1 rounded text-xs font-medium ${
                        ...
                        : i === figIdx ? "bg-[#2D8C6A] dark:bg-emerald-600 text-white"
              ...
              {figures[figIdx] && (
                <div className="card p-3 sticky top-20">
                  ...
                  {figures[figIdx].image_url ? (
                    <img src={figures[figIdx].image_url} alt={figures[figIdx].title} loading="lazy" onClick={() => setLightboxImg(figures[figIdx].image_url || null)} className="w-full rounded-lg mb-3 border border-[#E8ECF0] dark:border-slate-700 cursor-zoom-in hover:opacity-95 transition-opacity" />
                    ...
                    <div className="bg-[#F5F8FC] dark:bg-slate-800 rounded-lg mb-3 p-4 border border-dashed border-[#C5D3E0] dark:border-slate-600">
                      <div className="flex items-start gap-3">
                        <div className="w-10 h-10 rounded-full bg-[#E8F4F0] dark:bg-emerald-900/40 flex items-center justify-center text-lg shrink-0">👤</div>
                  ...
                  {figures.length > 1 && (
                    <div className="flex items-center justify-between text-xs">
                      <button onClick={() => void jumpToFigure(figIdx - 1)} disabled={figIdx === 0 || introGenerating} className="text-[#2D8C6A] dark:text-emerald-400 disabled:opacity-30 hover:underline">← 上一张</button>
                      <span className="text-[#8FA0B4] dark:text-slate-500">{figIdx + 1}/{figures.length}</span>
                      <button onClick={() => void handleNextFigure()} disabled={introGenerating} className="text-[#2D8C6A] dark:text-emerald-400 hover:underline">下一张 →</button>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="card">
                <div ref={chatRef} onScroll={handleChatScroll} className="h-[400px] sm:h-[450px] overflow-y-auto mb-4 space-y-3 pr-2">
                      ...
                      <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                        ...
                        {msg.role === "assistant" && !msg._uiOnly && (
                          <div className="flex items-center gap-1 mt-2 pt-1.5 border-t border-[#DDE5EE] dark:border-slate-600">
                            <button onClick={() => handleFeedback(i, "up")} className={`text-xs px-1.5 py-0.5 rounded transition-colors ${feedbackMap[i] === "up" ? "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400" : "text-[#8FA0B4] dark:text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400"}`}>👍</button>
                            <button onClick={() => handleFeedback(i, "down")} className={`text-xs px-1.5 py-0.5 rounded transition-colors ${feedbackMap[i] === "down" ? "bg-red-100 dark:bg-red-900/40 text-red-500 dark:text-red-400" : "text-[#8FA0B4] dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400"}`}>👎</button>
                          </div>
                        )}
                      </div>
                      {msg.role === "user" && (
                  ...
                  {streamingText !== null && (
                    <div className="flex gap-2 justify-start">
                      <div className="w-7 h-7 rounded-full bg-[#2D8C6A] dark:bg-emerald-600 flex items-center justify-center text-white text-xs font-bold shrink-0 mt-0.5">📈</div>
                      <div className="bg-[#F5F8FC] dark:bg-slate-800 border border-[#DDE5EE] dark:border-slate-700 rounded-2xl rounded-bl-md px-4 py-2.5 text-sm leading-relaxed text-[#3D5166] dark:text-slate-300 max-w-[80%]"><Markdown text={streamingText} /><span className="inline-block w-1.5 h-4 bg-[#2D8C6A] dark:bg-emerald-400 ml-0.5 animate-pulse align-middle" /></div>
                    </div>
                  )}
                  {(sending || introGenerating) && streamingText === null && (
                    <div className="flex gap-2 justify-start">
                      ...
                      <div className="bg-[#F5F8FC] dark:bg-slate-800 border border-[#DDE5EE] dark:border-slate-700 rounded-2xl rounded-bl-md px-4 py-3 flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-[#8FA0B4] dark:bg-slate-500 animate-bounce" style={{ animationDelay: "0ms" }} /><span className="w-2 h-2 rounded-full bg-[#8FA0B4] dark:bg-slate-500 animate-bounce" style={{ animationDelay: "150ms" }} /><span className="w-2 h-2 rounded-full bg-[#8FA0B4] dark:bg-slate-500 animate-bounce" style={{ animationDelay: "300ms" }} />
                      </div>
                    </div>
                  )}
                </div>
                <div className="flex gap-2">
                  <textarea value={input} onChange={(e) => setInput(e.target.value)} onCompositionStart={() => setIsComposing(true)} onCompositionEnd={() => setIsComposing(false)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey && !isComposing) { e.preventDefault(); handleSend(); } }} placeholder="输入你的判读分析...（Enter 发送，Shift+Enter 换行）" rows={4} disabled={sending || introGenerating} className="flex-1 px-3 py-2 bg-white dark:bg-slate-800 border border-[#C5D3E0] dark:border-slate-600 rounded-lg text-sm text-[#1A2332] dark:text-slate-100 placeholder-[#8FA0B4] dark:placeholder-slate-500 focus:outline-none focus:border-[#2D8C6A] dark:focus:border-emerald-400 resize-none" />
                  <button onClick={handleSend} disabled={sending || introGenerating || !input.trim()} className="self-end px-4 py-2 bg-[#2D8C6A] dark:bg-emerald-600 text-white font-medium rounded-lg hover:bg-[#1A6B4F] dark:hover:bg-emerald-500 transition-colors disabled:opacity-50 text-sm">发送</button>
                </div>
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  <button onClick={handleHint} disabled={sending} className="text-xs px-3 py-1.5 bg-[#FEF3E2] dark:bg-amber-900/20 text-[#854F0B] dark:text-amber-300 rounded-full hover:bg-amber-100 dark:hover:bg-amber-900/40 transition-colors disabled:opacity-40">💡 给我提示</button>
                  <button onClick={handleRestart} disabled={sending} className="text-xs px-3 py-1.5 bg-[#F5F8FC] dark:bg-slate-800 text-[#6B7F96] dark:text-slate-400 rounded-full hover:bg-[#E8ECF0] dark:hover:bg-slate-700 transition-colors disabled:opacity-40">🔄 重新开始</button>
                  <button onClick={handleEvaluate} disabled={sending || messages.length < 3} className="text-xs px-3 py-1.5 bg-[#EDE9FB] dark:bg-purple-900/20 text-[#4C3D9E] dark:text-purple-300 rounded-full hover:bg-purple-100 dark:hover:bg-purple-900/40 transition-colors disabled:opacity-40">📊 评估我</button>
                  <button onClick={handleCopyConversation} disabled={messages.length < 2} className="text-xs px-3 py-1.5 bg-[#E8F4F0] dark:bg-emerald-900/20 text-[#0F6E56] dark:text-emerald-300 rounded-full hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors disabled:opacity-40">{copied ? "✅ 已复制" : "📋 复制对话"}</button>
                </div>
                {figures.length > 0 && figIdx < figures.length - 1 && (
                  <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#E8ECF0] dark:border-slate-700">
                    ...
                    <button onClick={handleNextFigure} disabled={introGenerating} className="text-xs px-3 py-1 bg-[#E8F5F0] dark:bg-slate-700 text-[#2D8C6A] dark:text-emerald-400 rounded-lg hover:bg-[#2D8C6A] dark:hover:bg-emerald-600 hover:text-white transition-colors">下一步 →</button>
                  </div>
                )}
                {figures.length > 0 && figIdx === figures.length - 1 && (
                  <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#E8ECF0] dark:border-slate-700">
                    <span className="text-xs text-[#8FA0B4] dark:text-slate-500">所有步骤已完成：</span>
                    <button onClick={handleNextFigure} className="text-xs px-3 py-1 bg-[#E8F4F0] dark:bg-emerald-900/30 text-[#0F6E56] dark:text-emerald-300 rounded-lg hover:bg-[#0F6E56] dark:hover:bg-emerald-700 hover:text-white transition-colors">🎉 查看总结 →</button>
                  </div>
                )}
              </div>
            </div>
      ...
      {/* Image lightbox */}
      {lightboxImg && (
        <div onClick={() => setLightboxImg(null)} className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4 cursor-zoom-out">
          <button onClick={() => setLightboxImg(null)} className="absolute top-4 right-4 text-white text-3xl w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors">✕</button>
          <img src={lightboxImg} alt="放大查看" loading="lazy" className="max-w-[95vw] max-h-[95vh] object-contain rounded-lg shadow-2xl" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
    </AppLayout>
  );
