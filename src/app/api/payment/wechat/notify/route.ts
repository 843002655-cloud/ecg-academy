...
function decryptResource(_resource: WeChatPayNotification["resource"]): WeChatPayTransaction | null {
  // 完整 AES-GCM 解密需商户 API 证书；此处留作后续接入，回调可先走 attach 明文模式或管理员开通
  return null;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    let transaction = decryptResource(payload.resource);

    // 开发/过渡期：允许在请求体中直接携带 transaction（仅 DEBUG 模式）
